...
class VQAEvaluator:
    ...
    def _output_metrics_at_precision_intervals(precision: np.ndarray, recall: np.ndarray, thresholds: np.ndarray, label: int) -> None:
        ...
        target_precisions = np.array([0.100, 0.500, 0.750, 0.900, 0.950, 0.990, 0.995, 0.999])
    ...
    @staticmethod
    def _output_metrics_at_threshold_intervals(precision: np.ndarray, recall: np.ndarray, thresholds: np.ndarray, label: int) -> None:
        """Output metrics at regular threshold intervals of 0.05."""
        eps = 1e-12
        f1_scores = 2 * (precision * recall) / (precision + recall + eps)
        
        # Create precision intervals
        target_thresholds = np.array([0.100, 0.500, 0.700, 0.900, 0.950, 0.990, 0.999])
        
        logger.info(f"\nMetrics at threshold intervals for Label_{label}:")
        logger.info("Precision\tRecall\tF1\tThreshold")
        logger.info("-" * 50)
        ...
        for target_threshold in target_thresholds:
            ...
            if idx < len(precision) and idx < len(recall) and idx < len(f1_scores) and idx < len(thresholds):
                valid_indices.append((target_threshold, idx))
        
        # Output metrics for valid indices
        for target_threshold, idx in valid_indices:
            logger.info(f"{precision[idx]:.4f}\t{recall[idx]:.4f}\t{f1_scores[idx]:.4f}\t{thresholds[idx]:.4f}")
    ...
    def evaluate_white_samples(self, y_id: List[str], y_audit_label: List[str], 
        ...
        self._output_metrics_at_threshold_intervals(precision, recall, thresholds, label)
        ...
        with open(output_file, 'w') as f:
            ...
            for id, score, true_label, audit_label in sorted_data:
                ...
                if data.get("images") and len(data["images"]) > 0:
                    url = os.path.join("http://9.223.241.48", data["images"][0])
