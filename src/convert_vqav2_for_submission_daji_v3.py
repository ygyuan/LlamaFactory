        ...
        if score >= th:
            ...
            if audit_label!='100':
                if celueid not in pre_celue_dic:
                    ...
                    pre_celue_dic[celueid] += 1
                tmp_label=get_label_type(audit_label, hit_label_dict)
            ...
            if audit_label!='100':
                ...
                url = "" #datalist[id]['image'].replace("/apdcephfs_qy3/share_301069248/data/video","http://9.22.25.210/xiaoshijie") 
                res = "\t".join([id, str(score), str(pre_label), str(tmp_label), celue, url, audit_label, pinlun, beijing])+"\n"
                louguohei_info_dict[tmp_label].append(res)
        confusion[pre_label,y_true[i]]+=1

    # 写入错误案例
    for i in range(6):
        for j, path in enumerate(louguohei_info_dict[i]):
            err99_file.writelines(path)
    errbai99_file.writelines(errbai_info_dict)
    hit_file.writelines(hit_info_dict)
