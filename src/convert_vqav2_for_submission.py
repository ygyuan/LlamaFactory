    ...
    for i, score in enumerate(y_scores):
        ...
        if score >= th:
            ...
            if audit_label!='100':
                ...
                celue=datalist[id]['celue']
                qs = datalist[id]["messages"][0]["content"].split("请问发帖内容是否")[0]
                # pinlun=qs.split("发帖内容:")[1].replace("\n","")
                pinlun = qs.split("当前评论:")[1].replace("\n", "")
                beijing=qs.replace("<image>\n","").replace("\n","")
