...
class CustomDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        # print(f"Processed inputs for index {index}: {inputs}")
        return inputs
    ...
    def collate_batch(self, instances):
        ...
        for instance in instances:
            if instance is None:
                return None
            batch = dict(input_ids=instance['input_ids'], attention_mask=instance['attention_mask'], pixel_values=instance['pixel_values'])
        return batch
...
def safe_process_sample(processor, data, index):
    ...
    for i, image in enumerate(images):
        try:
            ...
            return None  # Skip the entire sample if any image fails
    
    # textdata = {"type": "text", "text": data['messages'][0]['content'].replace("<image>", "")}
    textdata = {"type": "text", "text": data['conversations'][0]['value']}
    ...
    try:
        ...
        print(f"Warning: Failed to process text in sample {index}: {e}")
        return None
...
def main(
    ...
    for inputs, line in tqdm(zip(data_loader, questions), total=len(questions)):
        if inputs is None:
            print(f"Skipping sample {indx} due to invalid inputs")
            continue
        try:
            idx = line["id"]
            
            # Move inputs to the same device as the model and convert to model's dtype
            inputs['input_ids'] = inputs['input_ids'].to(model.device)
            inputs['attention_mask'] = inputs['attention_mask'].to(model.device)
            inputs['pixel_values'] = inputs['pixel_values'].to(model.device).to(model_dtype)
