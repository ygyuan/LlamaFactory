...
def get_confusion(th_list, mejson, datalist, y_true, y_id, y_audit_label, multi_y_scores, y_multi_true):
    ...
    for i, mscore in enumerate(multi_y_scores):
        ...
        if (pre_mlabel == 1 and score >= th_list[pre_mlabel]) or \
           (pre_mlabel == 2 and score >= th_list[pre_mlabel]) or \
           (pre_mlabel == 3 and score >= th_list[pre_mlabel]) or \
           (pre_mlabel == 4 and score >= th_list[pre_mlabel]) :
        #if (pre_mlabel != 0) and (mscore[0] <= 0.2):
            # print(pre_mlabel, score, th_list[pre_mlabel])
            # lab=get_label_type(audit_label, pre_label_dic)
        #if (pre_mlabel != 0):
        #if (pre_mlabel == 1 and score >= 0.970) or \
        #   (pre_mlabel == 2 and score >= 0.700) or \
        #   (pre_mlabel == 3 and score >= 0.700) or \
        #   (pre_mlabel == 4 and score >= 0.950) :
            pre_label=1
...
def calculate_metric(y_true, y_scores):
...
if __name__ == '__main__':
    ...
    for label in range(num_label):
        ...
        # precision, recall, thresholds = calculate_metric(y_true_idx, y_scores_idx)
        # pdb.set_trace()
