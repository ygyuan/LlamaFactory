...
def read_data(args):
    ...
    # results = {x['question_id']: 1.0 - x['score'][0] for x in results}
    # results = {x['question_id']: 1.0 - x['score'] for x in results}
    # results = {x['question_id']: '%.6f' % x['score'] for x in results}

    results_dict = {}
    for x in results:
        results_dict[x['question_id']] = float(x['score']) * 2
    ...
    print(f'total_results: {len(results_dict)}, total_split: {len(test_split)}, error_line: {error_line}')
    return results_dict, test_split, datalist
...
if __name__ == '__main__':
    ...
    for i, score in enumerate(y_scores):
        ...
        pic=1
        if 'text' in datalist[id]:
            pinlun = datalist[id]["text"]
        else:
            pinlun = datalist[id]["input"].split("<sep>")[0].split("评论内容是：")[1]
        beijing = ""
        ...
        if score >= th:
            ...
            if audit_label!='100':
                ...
                res = "\t".join([id, "{:.6f}".format(score), str(pre_label), str(tmp_label), celue, url, audit_label, pinlun, beijing])+"\n"
    ...
    for i in range(confusion.size()[0]):
        ...
        f1 = 2 * p * r / (p + r + eps)
        wrongs = confusion[i, :].sum().item() - confusion[i, i].item()
        print("Label {}: corrects {} wrongs {} precision {:.4f}, recall {:.4f}, f1 {:.4f}".format(i, confusion[i, i].item(), wrongs, p, r, f1))
