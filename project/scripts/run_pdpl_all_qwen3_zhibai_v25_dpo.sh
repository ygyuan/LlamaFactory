...
if [ ${stage} -le 0 ] && [ ${stop_stage} -ge 0 ]; then
	...
	TEMPLATE="qwen3"
	MODEL_NAME="Qwen3-0.6B-Instruct"
	MODEL_NAME_OR_PATH="saves/${MODEL_NAME}/sft-${TEMPLATE}-fangwusha_${TASK_NAME}/checkpoint-1000"

	DATASET="keywords-251001-251208_v4_dpo,qunliaokey_241125-250313_v2_dpo,shenhe-240601-250531_v5_keywords_llmverify_dpo,tongbao_chouyang_v2_dpo,zhongzi-nlpsimilarity-250613_v3_dpo,zhongzi-nlpsimilarity-250929_v3_dpo"
        # 使用环境变量传递参数并执行训练脚本
        GPUS=${GPUS} PER_DEVICE_BATCH_SIZE=${PER_DEVICE_BATCH_SIZE} GRADIENT_ACC=${GRADIENT_ACC} MAX_STEPS=${MAX_STEPS} SAVE_STEPS=${SAVE_STEPS} \
	TASK_NAME=${TASK_NAME} MODEL_NAME_OR_PATH=${MODEL_NAME_OR_PATH} FINETUNING_TYPE=${FINETUNING_TYPE} \
	TEMPLATE=${TEMPLATE} DATASET=${DATASET} \
        bash project/scripts/run_pdpl_train_qwen3_zhibai_dpo_accelarate.sh
    done
fi
...
if [ ${stage} -le 2 ] && [ ${stop_stage} -ge 2 ]; then
	...
	for checkpoint in 25000 15000; do
            bash project/scripts/run_all_eval_qwen3_06b_prompt1_dpo.sh ${x} ${checkpoint} 1 1
        done
    done
fi
