...
if [ ${stage} -le 1 ] && [ ${stop_stage} -ge 1 ]; then
	...
	TEMPLATE=${TEMPLATE} DATASET=${DATASET} \
        bash project/scripts/run_textevents_train_qwen3_14b_full_accelarate.sh
    done
fi
...
if [ ${stage} -le 2 ] && [ ${stop_stage} -ge 2 ]; then
	...
	# for checkpoint in 18500 15000 10000 5000; do
        for checkpoint in 3000 500; do
            bash project/scripts/run_textevents_eval_qwen3_14b_prompt1.sh ${x} ${checkpoint} 1 1
        done
    done
fi
...
if [ ${stage} -le 3 ] && [ ${stop_stage} -ge 3 ]; then
    echo "start stage ${stage}, stop stage ${stop_stage}"
    for x in train_all_v3; do
        for checkpoint in 18500 5000; do
            bash project/scripts/run_textevents_eval_qwen3_14b_prompt1.sh ${x} ${checkpoint} 3 3
        done
    done
fi
