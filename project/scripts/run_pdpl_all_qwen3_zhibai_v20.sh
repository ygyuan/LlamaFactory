...
if [ ${stage} -le 1 ] && [ ${stop_stage} -ge 1 ]; then
	...
	TEMPLATE="qwen3"
	DATASET="qunliaokey_241125-250313_prompt2_juhe_qwen3"
        # 使用环境变量传递参数并执行训练脚本
...
if [ ${stage} -le 2 ] && [ ${stop_stage} -ge 2 ]; then
	...
	for checkpoint in 1200 1000 800 600; do
            bash project/scripts/run_all_eval_qwen3_06b_prompt2.sh ${x} ${checkpoint} 1 1
        done
    done
fi
