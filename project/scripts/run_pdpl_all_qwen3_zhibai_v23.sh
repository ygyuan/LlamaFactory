...
if [ ${stage} -le 1 ] && [ ${stop_stage} -ge 1 ]; then
	...
	MODEL_NAME_OR_PATH="/apdcephfs_hzlf/share_303924399/huggingface/Qwen3-4B"
	FINETUNING_TYPE="full"
	TEMPLATE="qwen3"
	DATASET="qunliaokey_241125-250313_juhe_qwen3,keyword_merge_all_251015_single_eyi_juhe_qwen3,shenhe-240601-250531_v4_keywords_normal_prompt4_juhe_qwen3"
        # 使用环境变量传递参数并执行训练脚本
...
if [ ${stage} -le 2 ] && [ ${stop_stage} -ge 2 ]; then
	...
	for checkpoint in 1000 500; do
            bash project/scripts/run_all_eval_qwen3_4b_prompt1.sh ${x} ${checkpoint} 1 1
        done
    done
fi
